...
export class PrismaReportRepository implements ReportRepository {
  ...
  async findMany(filters: ReportFilters) {
    const where = {
      ...(filters.proyectoId ? { proyecto_id: filters.proyectoId } : {}),
      ...(filters.estadoTicket ? { estado_ticket: filters.estadoTicket } : {}),
      ...(filters.tecnico ? { tecnico: { contains: filters.tecnico, mode: "insensitive" as const } } : {}),
      ...(filters.query
        ? {
            OR: [
              { cliente: { contains: filters.query, mode: "insensitive" as const } },
              { sucursal: { contains: filters.query, mode: "insensitive" as const } },
              { numero_ticket: { contains: filters.query, mode: "insensitive" as const } },
              { tecnico: { contains: filters.query, mode: "insensitive" as const } },
              { descripcion: { contains: filters.query, mode: "insensitive" as const } },
            ],
          }
        : {}),
  ...
  async update(id: string, input: UpdateReportInput): Promise<Report | null> {
  ...
  private toDbInput(input: CreateReportInput | UpdateReportInput) {
    return {
